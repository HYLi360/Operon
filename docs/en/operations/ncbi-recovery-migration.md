...
This procedure migrates a long-running legacy `operon` database to database schema 2.9 and metadata schema 1.4, and repairs the annotation duplication, GCA/GCF canonical drift, and QC state downgrades the old NCBI Datasets adapter may have left behind. The whole procedure respects these boundaries:
...
- `schema_version` is `2.9`;
- `integrity_check` is `ok`;
- `foreign_key_violations` is `0`;
- The migration ledger contains `2.6-recovery-and-ncbi-identities`, `2.7-entity-lifecycle-retirement`, `2.8-execution-environments`, and `2.9-lineage-recipes-resources` (a database already at a newer version only gains the missing entries).
