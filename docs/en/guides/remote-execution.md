...
- `analyze` uploads local inputs over SFTP. If `sha256sum` is unavailable remotely, SHA-256 is calculated through the SFTP stream. Directories use a full deterministic tree hash; size-only checks are never used. Different existing content is not overwritten.
- With `storage_remote`, a `REMOTE_ONLY` input is checked against local SQLite, the remote manifest, and actual remote content, then consumed in place under the remote root instead of being downloaded.
...
- On SSH direct-command timeout, Operon uses a permission-restricted remote PID file to send TERM and then KILL to the process group. If the PID file or termination command is unavailable, the error states that the remote process may still be running. Remote Slurm uses `scancel`.
...
> The automated tests for Slurm and SSH use simulated sbatch/squeue and in-memory SSH/SFTP implementations. Before production use on an HPC cluster, run a short task such as `run-external --backend slurm/ssh --command 'echo ok'` to validate submission, polling, and output retrieval.
