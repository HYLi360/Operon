...
- `sources` names the hit sources the rules can reference. Each source declares:
  - `analysis`: the analysis name whose `analysis_alignments` rows count; only rows of the latest `completed` job per target file contribute. Other completed jobs for the same analysis+file pair are ignored, and a non-zero count is surfaced as `ignored_completed_jobs` in the run output and details.
...
- Fields resolve against the alignment columns (`subject_id`, `hit_rank`, `query_start`, `query_end`, `evalue`, `bitscore`, `percent_identity`, …), then the derived `span` field (`query_end - query_start + 1`) and `seqid` (the query id up to the first whitespace), and finally the keys of the hit row's `extra_json` — parser-specific fields such as `hit_type`, `incomplete`, `short_name`, or `i_evalue`. A missing field never satisfies a condition in any form: the test is False for every operator (including `!=`, `not_in`, and `exists`), a `not: {...}` negation of a missing-field condition is still False, and a missing-field disjunct inside `any: [...]` counts as unsatisfied. `between` requires numeric operands and raises a validation error naming the field and value otherwise; `in`/`not_in` compare values as strings.
...
The [TUI's Config screen](../reference/cli-tui.md) edits these profiles with the same forms, version bump and content-addressed snapshot recording as the CLI (a later `classify-sequences` consumes exactly the saved snapshot); a profile whose conditions nest deeper than one `any:`/`not:` level opens read-only there so the form never rewrites structure it cannot represent.
