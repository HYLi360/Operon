...
The normal path enforces the legal transition table and appends a `changes` audit row with the message; `--force` bypasses the transition check for manual recovery while the audit row keeps the action traceable. `RELEASED` is not terminal for every command: rerunning `qc`/`evaluate` (which write state through the forced bulk path) or `standardize` (which writes state directly) can move an entity out of it without `--force`, and setting a state equal to the current one still rewrites the `entity_state` message and timestamp even though no audit row is written — see [Implicit behaviors, edge cases, and known issues](../reference/behaviors-and-limitations.md). Prefer `curate` (for decisions) or rerunning the affected step (for provenance) whenever possible.
