...
- The database checks run under `--dry-run` too: a preview reports the error a real run would hit — `reference database not found: <path>; edit config/tools.yaml` (local) or `remote reference database is not provisioned at <path>` (remote) — instead of `planned` for a database that exists nowhere. Only the tool version stays unprobed on a non-local backend;
