...
- **Slurm `strict` environment policy degrades to `warn`.** Slurm and remote-Slurm backends have no pre-job probe, so the environment comparison cannot run before a cache hit is reused; `environment_policy: strict` is downgraded to `warn` and the run details record `environment_policy_degraded: strict→warn` (`tools.py`).
- **Legacy environment documents compare as `unavailable`.** Captures recorded before database schema {{ db_schema }} carry no sub-fingerprints, so when either side of a cache-hit comparison lacks them the run details record `environment_compare: unavailable`: `warn` reuses as usual and `strict` degrades to `warn` rather than invalidating the cache (`environment.py`, `tools.py`).
