...
- Archive names (entity ID, role, and format) must be nonempty filename components without `/`, `\`, control characters, or standalone `.`/`..`. Ingestion and adoption reject destinations that resolve outside the project root, including symlink escapes and reused manifest paths, before writing.
...
- The assembly FASTA `seqid -> length` map is a rebuildable cache under `qc/cache/fasta_lengths/`, reused only for the same complete content identity; `--rehash` revalidates the source SHA-256 but can still reuse the verified index. See the [QC pipeline](../architecture/qc-and-rules.md#qc-pipeline).
- Every measured FASTA also syncs its `seqid -> length` map into the `sequences` table (`file_id + file_sha256 + entity + seqid + length`, reusing the same length cache), which powers seqid reverse lookup in `show` and `query`. Annotation GFF3 QC additionally syncs the sequences of the assembly FASTA it reads. Rows for a file are replaced wholesale on each successful measurement.
- Each processed file prints a progress line `[i/N] file_id: OK` or `[i/N] file_id: FAILED (<error>)`.
...
- Text cells a spreadsheet would execute are escaped with a leading apostrophe (a value beginning with `=`, `+`, `-`, `@`, TAB or CR), so `safe_id` can legitimately carry an escaped FASTA header; both alignment backends escape identically (ODR-0040).
