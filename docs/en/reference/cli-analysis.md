...
5. Skips execution when the completed-job cache in `analysis_jobs` matches, unless `--force` is used. If the exact fingerprint misses but an old completed job has the same input and an output whose hash verifies, the output is adopted under the current fingerprint and reported as `adopted` instead of recomputed. On a cache hit, a recipe whose `environment_policy` is not `ignore` additionally compares the environment-relevance fingerprint, warning on a mismatch (or recomputing under `strict`); see "Environment policy on cache reuse" in the [Recipe field reference](recipe-fields.md).
...
`--dry-run` lists the plan without execution. Status values are `cached` (completed cache hit), `adoptable` (verified old output will be adopted), or `planned` (execution will run). The output column contains planned output paths, and `tool_version` contains the detected version on the local backend; a non-local backend reports `not probed (backend=…)` because the probe itself would open a connection. A dry run does perform the validations that need no command execution — the recipe database's local existence, a remote reference database's provisioning (a stat over the backend), and the `database_checksum` rule — so a plan that cannot execute reports the same error a real run would, and the command exits non-zero.
...
- Target files are the manifest files matching the profile's `applies_to.entity_type` + `applies_to.file_role`; superseded and effectively retired entities are excluded. Per file, each seqid registered in the `sequences` table is classified against the hits of the latest `completed` job per declared source analysis in `analysis_alignments`. When other completed jobs exist for the same analysis+file pair they are ignored, and a non-zero count is surfaced as `ignored_completed_jobs` in the output and in the run's `execution_details`; files absent from the `sequences` registry are skipped and the skip count is printed.
- Rules are evaluated in order and the first match wins; a sequence matched by no rule (and no `default`) stays unlabeled. A missing field never satisfies a condition in any form — including inside a `not:` negation or an `any:` group. Each label records its decision evidence (rule index, source, job/alignment id, observed values) in `details_json`.
...
- `taxon` resolves a scientific name to TimeTree/NCBI taxonomy candidates (`taxon_id`, `scientific_name`, `rank`). `pairwise` reports the divergence-time summary for exactly two taxa; `mrca` for the MRCA of N taxa; `timeline` lists the node timetable from one taxon back to the last universal ancestor. Taxa are given as repeated `--taxon NAME` / `--taxon-id N` (or comma-separated `--taxa` for `mrca` and `calibrations`); an ambiguous name is never auto-resolved — the error lists the candidates and asks for `--taxon-id`. All query commands accept `--format text|json` (default `text`), `--refresh` to bypass the cache, and the network controls `--timeout` (seconds, default 30), `--retries` (default 3), and `--delay` (seconds between real requests, default 0.5).
...
- `fetch` and `calibrate` are project-independent and work on explicit file paths: `fetch` downloads the summary and per-study evidence for selected NCBI taxon pairs into a new immutable snapshot directory (raw responses plus a checksummed manifest, never overwriting a prior run; `--timeout` defaults to 30 seconds and `--retries` to 3); `calibrate` compiles reviewed soft bounds from such a snapshot onto a rooted, strictly bifurcating species tree (every constraint needs `approved=yes` and a rationale; summary confidence intervals are evidence, never fossil bounds). `--unit-ma` (default 100) states how many Ma one time unit represents: every bound is divided by it, so `min_ma=20, max_ma=30` with `unit_ma=100` is encoded as `B(0.2,0.3)`.
