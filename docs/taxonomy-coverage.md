...
本功能回答两个不同但相关的问题：项目元数据登记了哪些科/属，以及某个不可变
release 实际发布了哪些科/属。两种口径都只针对一个预先编译、带校验和的分母快照
计算；运行报告时不会实时遍历“最新版”taxonomy，因此历史数字不会随 NCBI
Taxonomy 升级而漂移。

当前实现只支持 **NCBI Taxonomy**。GTDB TaxID 不参与分子计算，也不会被按名称猜测
映射；GTDB 支持属于后续扩展边界。
