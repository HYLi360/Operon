...
| `file_role_prefix` | 空 | 对 `files.file_role` 做 `:` 边界前缀匹配——选中精确 role 与所有 `<prefix>:*`；与 `file_role` 互斥 |
...
`file_role` 与 `file_role_prefix` 互斥。前缀按 `:` 边界匹配，而不是纯字符前缀：
`file_role_prefix: "sub"` 选中精确的 `sub` 与所有 `sub:*`，但不会选中 `sub2:*`。
它也不是模式——通配符（`%`、`*`、`?`）在 recipe 加载时即被拒绝——因此
