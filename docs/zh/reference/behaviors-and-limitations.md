...
- **TUI 仅以本地/项目默认后端发起分析，取消在文件边界生效。** TUI 的分析对话框不提供 Slurm/SSH 启动（这些请用 CLI）；其 Cancel 按钮/Escape 在进度回调中抛出取消异常，因此进行中的文件会跑完，批量在下一个文件边界停止——在 Slurm array 路径上，该异常会中止结果收集并保留已完成的任务，而不再被记为逐文件失败。核心还接受协作式 `cancel_event`（`run_analysis(..., cancel_event=...)`）：置位后批量在下一个文件/规划/收集边界停止；接受该事件的 Slurm/SSH 执行器会对仍在队列中的作业或 array 执行一次 `scancel`。所有取消路径都与信号走相同的中断收尾（`interrupted` 作业行、部分产物删除、下次运行时清扫陈旧 RUNNING 行）（`tui/actions.py`、`tools.py`、`execution.py`）。
