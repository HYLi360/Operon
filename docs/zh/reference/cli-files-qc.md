...
- 会被电子表格当作公式执行的文本单元格会加前导撇号转义（即以 `=`、`+`、`-`、`@`、TAB 或 CR 开头的值），因此 `safe_id` 可以合法承载被转义的 FASTA 头部；两个对齐后端的转义完全一致（ODR-0040）。
- 空比对或序列长度不一致会以错误和非零退出码拒绝。
- 核心计算是对 `(header, sequence)` 记录的单次遍历纯函数（`compute_alignment_qc`）。`operon/qc/_alignment.pyx` 的 Cython 构建是生产后端；`operon/qc/alignment.py` 是纯 Python 参考实现，Cython 后端必须逐字节复现其结果（parity 由回归测试保证）。
