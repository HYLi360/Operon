...
内置原始图片为 1024 × 768 像素。为兼容普通终端模拟器，Textual 使用彩色
Unicode 半块字符绘制预采样资源，根据终端尺寸保持 4:3 场景比例（假定字符格
高度为宽度的两倍）。副标题使用终端文本，确保小尺寸下可读。真彩色终端效果
最佳，颜色有限的终端会缩减调色板。无需 Kitty/Sixel 图片协议或额外图像依赖。
实际细节受终端字符分辨率限制，并非逐像素显示 1024 × 768 的图片。
