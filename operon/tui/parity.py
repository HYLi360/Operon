...
REGISTRY: tuple[ParityEntry, ...] = (
    ...
    ParityEntry(
        ("ncbi-datasets",),
        STATUS_IMPLEMENTED,
        note="the dry run is the mandatory preflight: offline inputs are parsed "
        "and planned without writes, accession-only requests show the download "
        "plan (--plan-only); Confirm unlocks after a clean preflight, and Cancel "
        "stops a run cooperatively (recorded as interrupted, resumable)",
        actions="actions.ncbi_datasets",
        modal="operon.tui.screens.ncbi_datasets::NcbiDatasetsModal",
        params={
            "inputs": "ncbi-inputs (comma separated -> repeated --input)",
            "accession": "ncbi-accessions (comma separated -> repeated --accession)",
            "accession_file": "ncbi-accession-file",
            "include": "ncbi-include (comma separated -> repeated --include)",
            "no_archive_files": "ncbi-no-archive-files",
            "standardize": "ncbi-standardize",
            "dry_run": "ncbi-dry-run / ncbi-preflight-button (mandatory preflight)",
            "no_preserve_source": "ncbi-no-preserve-source",
            "email": "ncbi-email",
            "api_key": "ncbi-api-key",
            "timeout": "ncbi-timeout",
            "batch_size": "ncbi-batch-size",
            "download_workers": "ncbi-download-workers",
            "retries": "ncbi-retries",
            "retry_backoff": "ncbi-retry-backoff",
            "resume_run": "ncbi-resume-run",
    ...
    ParityEntry(
        ("extract-domains",),
        STATUS_IMPLEMENTED,
        note="the output FASTA stays unregistered; the modal chains into the adopt dialog",
        modal="operon.tui.screens.derived_ops::ExtractModal",
        actions="actions.extract_domains",
        params={
            "file_id": "context: the selected Files row",
            "analysis": "extract-analysis",
            "regions_tsv": "extract-regions-tsv",
            "flank": "extract-flank",
            "min_length": "extract-min-length",
            "all_regions": "extract-region-mode (best hit only / all regions)",
            "best_only": "extract-region-mode (best hit only / all regions)",
            "subject_like": "extract-subject-like",
            "evalue_max": "extract-evalue-max",
            "out": "extract-out",
    ...
    ParityEntry(
        ...
        STATUS_IMPLEMENTED,
        note="the output FASTA stays unregistered; the modal chains into the adopt dialog",
        modal="operon.tui.screens.derived_ops::SelectSequencesModal",
        actions="actions.select_sequences",
        params={
            "file_id": "context: the selected Files row",
            "analysis": "select-analysis-1 / select-analysis-2",
            "subject_like": "select-subject-like",
            "evalue_max": "select-evalue-max",
            "min_span": "select-min-span",
            "hit_type": "select-hit-type",
            "require_hit": "select-requirement (require a hit / require no hit)",
            "require_no_hit": "select-requirement (require a hit / require no hit)",
            "entity_type": "select-entity-type",
            "entity_id": "select-entity-id",
            "out": "select-out",
            "manifest": "select-manifest",
        },
    ),
    ParityEntry(
        ("classify-sequences",),
        STATUS_IMPLEMENTED,
        note="the run reads the selected profile from disk — the same core call, "
        "transaction and idempotency contract as the CLI",
        modal="operon.tui.screens.classify::ClassifyModal",
        actions="actions.run_classify",
        params={
            "profile": "context: the Config screen's selected classification profile",
        },
    ),
    ParityEntry(
        ("adopt",),
        STATUS_IMPLEMENTED,
        note="single-item and manifest modes; the same reuse/conflict contract as the CLI",
        modal="operon.tui.screens.derived_ops::AdoptModal",
        actions="actions.adopt",
        params={
            "file": "adopt-path (single-item mode)",
            "manifest": "adopt-manifest (manifest mode, after the preview)",
            "entity_type": "adopt-entity-type",
            "entity_id": "adopt-entity-id",
            "role": "adopt-role",
            "fmt": "adopt-format",
            "compression": "adopt-compression",
            "derived_from": "adopt-derived-from (comma separated)",
            "workflow_run_id": "adopt-workflow-run-id",
            "actor": "adopt-actor",
        },
    ),
    ParityEntry(
        ("fanout",),
        STATUS_IMPLEMENTED,
        note="the dry run is the mandatory first step: it performs the real preflight, "
        "shows would_create/would_reuse, and writes nothing until Confirm",
        modal="operon.tui.screens.derived_ops::FanoutModal",
        actions="actions.fanout",
        params={
            "assignments_file": "fanout-assignments",
            "source_file": "fanout-sources (comma separated)",
            "entity_type": "fanout-entity-type",
            "entity_id": "fanout-entity-id",
            "role_prefix": "fanout-role-prefix",
            "unit_column": "fanout-unit-column",
            "seqid_column": "fanout-seqid-column",
            "parent_run_id": "fanout-parent-run",
            "actor": "fanout-actor",
            "dry_run": "fanout-preview-button (mandatory preflight before Confirm)",
        },
    ),
    ParityEntry(
        ("report", "analysis"),
