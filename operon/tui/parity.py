...
REGISTRY: tuple[ParityEntry, ...] = (
    ...
    ParityEntry(
        ("profiles", "history"),
        STATUS_IMPLEMENTED,
        note="qc-kind profiles only; sequence_classification profiles are "
        f"{_M3} and taxonomy_coverage profiles are {_M4}",
    ...
    ParityEntry(
        ("workflow", "list"),
        STATUS_IMPLEMENTED,
        note="partial: RunsPanel filters cover status/step/entity/limit only",
        ...
        waived={
            "started_from": f"no time-range filter in the Runs screen ({_M2B})",
            "started_to": f"no time-range filter in the Runs screen ({_M2B})",
            "run_id": f"single-run drill-down is the run detail screen ({_M2B})",
            "parent_run_id": f"no lineage filters in the Runs screen ({_M2B})",
            "resumes_run_id": f"no lineage filters in the Runs screen ({_M2B})",
            "tool": f"no tool filter in the Runs screen ({_M2B})",
            "executor": f"no executor filter in the Runs screen ({_M2B})",
            "offset": f"the Runs screen pages by limit only ({_M2B})",
            "oldest_first": f"the Runs screen is newest-first only ({_M2B})",
    ...
    ParityEntry(
        ("workflow", "show"),
        STATUS_IMPLEMENTED,
        note="partial: RunDetailScreen without streaming or JSON output",
        actions="data.workflow_run_detail",
        waived={
            "format": f"the TUI renders a detail view, not JSON ({_M2B})",
            "follow": f"no streaming log follow in the TUI ({_M2B})",
        },
    ),
    ParityEntry(
        ("report", "decisions"),
    ...
    ParityEntry(("run-external",), STATUS_PLANNED, note=_M2B),
    ...
    ParityEntry(("report", "analysis"), STATUS_PLANNED, note=_M3),
