...
TRANSITIONS: dict[str, set[str]] = {
    ...
    "CHECKSUM_FAILED": {"DOWNLOAD_PENDING", "DOWNLOADED", "STANDARDIZED"},
...
def read_log_tail(path: Path, offset: int = 0) -> tuple[str, int]:
    """Read the content appended to *path* since *offset*.

    Returns the decoded text and the offset to resume from. A missing file
    yields empty text; a truncated or rotated file is re-read from the start.
    """
    try:
...
def follow_run_logs(
    ...
    Every backend (local/slurm/ssh) writes the same ``logs/<run_id>.*.log``
    pair, so following works uniformly. ``status_getter`` is polled every
    ``poll_interval`` seconds and must return the current ``workflow_runs``
    row (or ``None``); only the ``running`` status keeps the loop alive. Once
    the run reaches any other status both logs are drained to EOF and a final
    status line is printed. Returns 0 when the run completed, 1 when it
    failed or vanished, and 130 on Ctrl-C — following only observes and never
    ...
    try:
        while True:
            drain()
            record = status_getter()
            if record is None:
                print(f"workflow run disappeared while following: {run_id}", file=out)
                out.flush()
                return 1
        ...
        print(f"\nstopped following {run_id}; the run itself keeps going", file=out)
        out.flush()
        return 130
...
def run_external_command(
    ...
    ``run_id`` may be pre-generated by the caller (e.g. to print log paths
    before a long run starts); when omitted a fresh one is allocated.
    """
